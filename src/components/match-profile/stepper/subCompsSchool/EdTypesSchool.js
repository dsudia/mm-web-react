...
export default class EdTypesSchool extends Component {
    handleNoChecked = (event, isInputChecked) => {
        if (isInputChecked) {
    ...
    handleSomeChecked = (event, isInputChecked) => {
        if (isInputChecked) {
    ...
    handleBachChecked = (event, isInputChecked) => {
        if (isInputChecked) {
    ...
    handleMastChecked = (event, isInputChecked) => {
        if (isInputChecked) {
    ...
    handleDocChecked = (event, isInputChecked) => {
        if (isInputChecked) {
        ...
        return (
            <div style={styles.block}>
                <Checkbox
                    ...
                    checked={this.props.currentUser.matchingProfile.edTypes.includes(
                        0
                    )}
                />
                <Checkbox
